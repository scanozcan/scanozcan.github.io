...
For a while, I was working on [CASPEX TF-Binding Zone Predictor](https://github.com/scanozcan/Caspex-binding-site-predictor), an R pipeline that predicts the transcription factor binding locations from CASPEX (GloPro) proximity-labeling proteomics enrichment data. The pipeline projects per-region logFC values onto a continuous promoter coordinate system, smoothing with a Gaussian kernel matched to the APEX labeling radius (~250 bp), normalizing for uneven guide RNA spacing, and decomposing the resulting landscape into discrete binding events anchored to TF motifs. The first version is now uploaded to github with an example dataset.
