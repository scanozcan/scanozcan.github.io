---
layout: post
title: "Some albums that I like and recommend"
date: 2026-02-06
categories: general
---

## Romantic

<div class="albums-grid">

  <div class="album-card">
    <img src="{{ '/assets/images/albums/brahms-intermezzi-rhapsodies.jpg' | relative_url }}" alt="Brahms: Intermezzi, Rhapsodies - François Chaplin" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Brahms: Intermezzi, Rhapsodies
        <span class="album-icons">
          <a href="https://app.idagio.com/albums/brahms-intermezzi-rhapsodies" style="text-decoration: none; color: inherit;" title="Listen on IDAGIO" class="idagio-album-icon">
            <svg width="20" height="20" viewBox="140 124 200 232" fill="currentColor" style="vertical-align: middle;"><path d="M231.998,344.0026 L231.998,296.0026 L328,296.0026 L328,344.0026 L231.998,344.0026 Z M152,136 L280,216 L152,296.002 L152,136 Z"/></svg>
          </a>
          <a href="https://open.spotify.com/album/3K9JpiiKMIMMGn8Oe1c6cV" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/110361124" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">François Chaplin</div>
      <div class="pub-venue">Little Tribeca, 2019</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/horowitz-golden-jubilee-concert.jpg' | relative_url }}" alt="Golden Jubilee Concert - Vladimir Horowitz" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Golden Jubilee Concert
        <span class="album-icons">
          <a href="https://open.spotify.com/album/5Cbmwmd1BOdFUCDCPN41cQ" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/album/59326391/u" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Vladimir Horowitz</div>
      <div class="pub-venue">BMG Entertainment, 2000</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/jan-lisiecki-chopin-complete-nocturnes.jpg' | relative_url }}" alt="Chopin: Complete Nocturnes - Jan Lisiecki" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Chopin: Complete Nocturnes
        <span class="album-icons">
          <a href="https://app.idagio.com/albums/chopin-complete-nocturnes-1CD7174C-F953-4177-9FEC-2556A8594B95" style="text-decoration: none; color: inherit;" title="Listen on IDAGIO" class="idagio-album-icon">
            <svg width="20" height="20" viewBox="140 124 200 232" fill="currentColor" style="vertical-align: middle;"><path d="M231.998,344.0026 L231.998,296.0026 L328,296.0026 L328,344.0026 L231.998,344.0026 Z M152,136 L280,216 L152,296.002 L152,136 Z"/></svg>
          </a>
          <a href="https://open.spotify.com/album/1mvFSxhBUzeOlOFU6SlhAw" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/193356298" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Jan Lisiecki</div>
      <div class="pub-venue">Deutsche Grammophon, 2021</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/lugansky-rachmaninov-24-preludes.jpg' | relative_url }}" alt="Rachmaninov: 24 Preludes - Nikolai Lugansky" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Rachmaninov: 24 Preludes
        <span class="album-icons">
          <a href="https://app.idagio.com/albums/rachmaninov-24-preludes" style="text-decoration: none; color: inherit;" title="Listen on IDAGIO" class="idagio-album-icon">
            <svg width="20" height="20" viewBox="140 124 200 232" fill="currentColor" style="vertical-align: middle;"><path d="M231.998,344.0026 L231.998,296.0026 L328,296.0026 L328,344.0026 L231.998,344.0026 Z M152,136 L280,216 L152,296.002 L152,136 Z"/></svg>
          </a>
          <a href="https://open.spotify.com/album/5j5EOBrr8lQSVTkCHZ07KQ" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/album/83515052/u" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Nikolai Lugansky</div>
      <div class="pub-venue">harmonia mundi, 2018</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/destination-rachmaninov-departure.jpg' | relative_url }}" alt="Destination Rachmaninov: Departure - Daniil Trifonov" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Destination Rachmaninov: Departure
        <span class="album-icons">
          <a href="https://app.idagio.com/albums/destination-rachmaninov-departure" style="text-decoration: none; color: inherit;" title="Listen on IDAGIO" class="idagio-album-icon">
            <svg width="20" height="20" viewBox="140 124 200 232" fill="currentColor" style="vertical-align: middle;"><path d="M231.998,344.0026 L231.998,296.0026 L328,296.0026 L328,344.0026 L231.998,344.0026 Z M152,136 L280,216 L152,296.002 L152,136 Z"/></svg>
          </a>
          <a href="https://open.spotify.com/album/5MFK8xu5ZnoKezAQQ5UFE5" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/96194798" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Daniil Trifonov, The Philadelphia Orchestra, Yannick Nézet-Séguin</div>
      <div class="pub-venue">Deutsche Grammophon, 2018</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/destination-rachmaninov-arrival.jpg' | relative_url }}" alt="Destination Rachmaninov: Arrival - Daniil Trifonov" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Destination Rachmaninov: Arrival
        <span class="album-icons">
          <a href="https://app.idagio.com/albums/destination-rachmaninov-arrival" style="text-decoration: none; color: inherit;" title="Listen on IDAGIO" class="idagio-album-icon">
            <svg width="20" height="20" viewBox="140 124 200 232" fill="currentColor" style="vertical-align: middle;"><path d="M231.998,344.0026 L231.998,296.0026 L328,296.0026 L328,344.0026 L231.998,344.0026 Z M152,136 L280,216 L152,296.002 L152,136 Z"/></svg>
          </a>
          <a href="https://open.spotify.com/album/1CjlWmQhGgkAKFiikkzEqL" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/119435720" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Daniil Trifonov, The Philadelphia Orchestra, Yannick Nézet-Séguin</div>
      <div class="pub-venue">Deutsche Grammophon, 2019</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/katsaris-bortkiewicz-piano-works.jpg' | relative_url }}" alt="Bortkiewicz: Piano Works - Cyprien Katsaris" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Bortkiewicz: Piano Works
        <span class="album-icons">
          <a href="https://open.spotify.com/album/4mAPLyahPTbGJFSDzLpMEw" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/album/57435463/u" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Cyprien Katsaris</div>
      <div class="pub-venue">Piano 21, 1995</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/bortkiewicz-piano-concertos-2-3.jpg' | relative_url }}" alt="Bortkiewicz: Piano Concertos 2 & 3 - Stefan Doniga" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Bortkiewicz: Piano Concertos 2 & 3
        <span class="album-icons">
          <a href="https://open.spotify.com/album/0efdC1vZgHK4ph5BWAFiv9" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/album/83909020/u" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Stefan Doniga, David Porcelijn, Janáček Philharmonic Orchestra</div>
      <div class="pub-venue">Piano Classics, 2018</div>
    </div>
  </div>

</div>

## Baroque

<div class="albums-grid">

  <div class="album-card">
    <img src="{{ '/assets/images/albums/pergolesi-stabat-mater.jpg' | relative_url }}" alt="Pergolesi: Stabat Mater" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Pergolesi: Stabat Mater
        <span class="album-icons">
          <a href="https://app.idagio.com/albums/pergolesi-stabat-mater-2395F7F8-F2F0-4CC0-A9B2-EC8BB25380ED" style="text-decoration: none; color: inherit;" title="Listen on IDAGIO" class="idagio-album-icon">
            <svg width="20" height="20" viewBox="140 124 200 232" fill="currentColor" style="vertical-align: middle;"><path d="M231.998,344.0026 L231.998,296.0026 L328,296.0026 L328,344.0026 L231.998,344.0026 Z M152,136 L280,216 L152,296.002 L152,136 Z"/></svg>
          </a>
          <a href="https://open.spotify.com/album/3h1xC2FoleDUGQD30MXaLi" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/209047963" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Jodie Devos, Adèle Charvet, Le Concert de la Loge, Julien Chauvin</div>
      <div class="pub-venue">Alpha Classics, 2022</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/yunchan-lim-goldberg-variations.jpg' | relative_url }}" alt="J.S. Bach: Goldberg Variations - Yunchan Lim" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        J.S. Bach: Goldberg Variations
        <span class="album-icons">
          <a href="https://app.idagio.com/albums/j-s-bach-goldberg-variations-live-at-carnegie-hall-new-york-2025" style="text-decoration: none; color: inherit;" title="Listen on IDAGIO" class="idagio-album-icon">
            <svg width="20" height="20" viewBox="140 124 200 232" fill="currentColor" style="vertical-align: middle;"><path d="M231.998,344.0026 L231.998,296.0026 L328,296.0026 L328,344.0026 L231.998,344.0026 Z M152,136 L280,216 L152,296.002 L152,136 Z"/></svg>
          </a>
          <a href="https://open.spotify.com/album/7EzK6PwHYbMgc8M9FVSG7s" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/album/494409220/u" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Yunchan Lim</div>
      <div class="pub-venue">Live at Carnegie Hall, New York, 2025 · Decca Classics, 2026</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/vikingur-olafsson-bach.jpg' | relative_url }}" alt="Johann Sebastian Bach - Víkingur Ólafsson" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Johann Sebastian Bach
        <span class="album-icons">
          <a href="https://app.idagio.com/albums/johann-sebastian-bach-2518F946-1C63-425A-9EA4-AD03B833BB0E" style="text-decoration: none; color: inherit;" title="Listen on IDAGIO" class="idagio-album-icon">
            <svg width="20" height="20" viewBox="140 124 200 232" fill="currentColor" style="vertical-align: middle;"><path d="M231.998,344.0026 L231.998,296.0026 L328,296.0026 L328,344.0026 L231.998,344.0026 Z M152,136 L280,216 L152,296.002 L152,136 Z"/></svg>
          </a>
          <a href="https://open.spotify.com/album/55XdhiY6nd3iij8msqiagR" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/94373347" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Víkingur Ólafsson</div>
      <div class="pub-venue">Deutsche Grammophon, 2018</div>
    </div>
  </div>

</div>

## Others

<div class="albums-grid">

  <div class="album-card">
    <img src="{{ '/assets/images/albums/foehn-trio-magnesie.jpg' | relative_url }}" alt="Magnésie - Foehn Trio" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Magnésie
        <span class="album-icons">
          <a href="https://open.spotify.com/album/7H8ymWCX4akkeO3TuxER8q" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/album/73387369/u" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Foehn Trio</div>
      <div class="pub-venue">Mad Chaman, 2017</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/foehn-soleil-de-minuit.jpg' | relative_url }}" alt="Soleil de Minuit - Foehn Trio" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Soleil de Minuit
        <span class="album-icons">
          <a href="https://open.spotify.com/album/1sYX3ErJCn6upVtMGxLBBX" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/album/490780409/u" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Foehn Trio</div>
      <div class="pub-venue">Kollision Records, 2026</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/olafur-arnalds-for-now-i-am-winter.jpg' | relative_url }}" alt="For Now I Am Winter - Ólafur Arnalds" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        For Now I Am Winter
        <span class="album-icons">
          <a href="https://open.spotify.com/album/5lssELel9HlvXJR0ndiqoy" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/album/330855983/u" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Ólafur Arnalds</div>
      <div class="pub-venue">Mercury KX, 2013</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/gogo-penguin-v2.jpg' | relative_url }}" alt="v2.0 - GoGo Penguin" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        v2.0
        <span class="album-icons">
          <a href="https://open.spotify.com/album/0ZvyleSEIlDbklGZ995APl" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/192743822" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">GoGo Penguin</div>
      <div class="pub-venue">Blue Note Records, 2014</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/gogo-penguin-a-humdrum-star.jpg' | relative_url }}" alt="A Humdrum Star - GoGo Penguin" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        A Humdrum Star
        <span class="album-icons">
          <a href="https://open.spotify.com/album/2UFElDP17YbtW2aVf1eGyI" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/95645815" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">GoGo Penguin</div>
      <div class="pub-venue">Blue Note Records, 2018</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/ibrahim-maalouf-kalthoum.jpg' | relative_url }}" alt="Kalthoum - Ibrahim Maalouf" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Kalthoum (Alf Leila Wa Leila)
        <span class="album-icons">
          <a href="https://open.spotify.com/album/1EoKR1J4RaDtkiEzAG7Fxb" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/51684551" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Ibrahim Maalouf</div>
      <div class="pub-venue">Mister Ibé, 2015</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/ibrahim-maalouf-levantine-symphony.jpg' | relative_url }}" alt="Levantine Symphony No. 1 - Ibrahim Maalouf" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Levantine Symphony No. 1
        <span class="album-icons">
          <a href="https://open.spotify.com/album/5NNkPdjdfPi5W3oQvHupcJ" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/122506886" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Ibrahim Maalouf</div>
      <div class="pub-venue">Mister Ibe, 2018</div>
    </div>
  </div>

  <div class="album-card">
    <img src="{{ '/assets/images/albums/dhafer-youssef-diwan-of-beauty-and-odd.jpg' | relative_url }}" alt="Diwan of Beauty and Odd - Dhafer Youssef" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Diwan of Beauty and Odd
        <span class="album-icons">
          <a href="https://open.spotify.com/album/3G531AVKSiGn2oU6UjrICy" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="https://tidal.com/browse/album/64498671" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Dhafer Youssef</div>
      <div class="pub-venue">Okeh, 2016</div>
    </div>
  </div>

</div>

<!-- Copy this block for each new album:

  <div class="album-card">
    <img src="{{ '/assets/images/albums/FILENAME.jpg' | relative_url }}" alt="Album Title" class="album-cover">
    <div class="album-info">
      <div class="pub-title">
        Album Title
        <span class="album-icons">
          <a href="IDAGIO_URL" style="text-decoration: none; color: inherit;" title="Listen on IDAGIO" class="idagio-album-icon">
            <svg width="20" height="20" viewBox="140 124 200 232" fill="currentColor" style="vertical-align: middle;"><path d="M231.998,344.0026 L231.998,296.0026 L328,296.0026 L328,344.0026 L231.998,344.0026 Z M152,136 L280,216 L152,296.002 L152,136 Z"/></svg>
          </a>
          <a href="SPOTIFY_URL" style="text-decoration: none; color: inherit;" title="Listen on Spotify" class="spotify-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="vertical-align: middle;"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>
          </a>
          <a href="TIDAL_URL" style="text-decoration: none; color: inherit;" title="Listen on TIDAL" class="tidal-album-icon">
            <svg width="18" height="18" viewBox="0 0 24 16" fill="currentColor" style="vertical-align: middle;"><path d="M4 0L8 4L4 8L0 4ZM12 0L16 4L12 8L8 4ZM20 0L24 4L20 8L16 4ZM12 8L16 12L12 16L8 12Z"/></svg>
          </a>
        </span>
      </div>
      <div class="pub-authors">Artist / Performer</div>
      <div class="pub-venue">Label, Year</div>
      <div class="pub-links">
        <span style="font-weight: normal;">Your note about this album.</span>
      </div>
    </div>
  </div>

  -->

<style>
  .albums-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 20px;
    margin-bottom: 40px;
  }

  .album-card {
    display: flex;
    gap: 14px;
    break-inside: avoid;
  }

  .album-cover {
    width: 120px;
    height: 120px;
    object-fit: cover;
    border-radius: 6px;
    flex-shrink: 0;
  }

  .album-info {
    flex: 1;
    min-width: 0;
  }

  .album-icons {
    margin-left: 8px;
    white-space: nowrap;
  }

  .idagio-album-icon svg, .spotify-album-icon svg, .tidal-album-icon svg {
    width: 18px;
    height: 18px;
    transition: all 0.2s cubic-bezier(0.34, 1.56, 0.64, 1);
    vertical-align: middle;
  }

  .idagio-album-icon {
    margin-right: 6px;
  }

  .spotify-album-icon {
    margin-right: 6px;
  }

  .spotify-album-icon:hover svg {
    color: #1DB954 !important;
    opacity: 0.9;
    transform: scale(1.15);
  }

  .idagio-album-icon:hover svg {
    color: #c0392b !important;
    opacity: 0.9;
    transform: scale(1.15);
  }

  .tidal-album-icon:hover svg {
    color: #C49A3C !important;
    opacity: 0.9;
    transform: scale(1.15);
  }

  @media (max-width: 768px) {
    .albums-grid {
      grid-template-columns: 1fr;
      gap: 15px;
    }

    .album-cover {
      width: 100px;
      height: 100px;
    }
  }
</style>
